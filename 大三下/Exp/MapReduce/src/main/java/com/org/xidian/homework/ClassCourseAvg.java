...
public class ClassCourseAvg {
    public static class GradeMapper extends Mapper<LongWritable, Text, Text, DoubleWritable> {
        ...
        protected void map(LongWritable key, Text value, Context context)
            ...
            try {
                ...
                classCourse.set(className + "\t" + courseName);
                scoreValue.set(score);
                context.write(classCourse, scoreValue);
    ...
    public static void main(String[] args) throws Exception {
        if (args.length != 2) {
            System.err.println("Usage: ClassCourseAvg <input> <output>");
            System.exit(2);
        }

        Configuration conf = new Configuration();
        Job job = Job.getInstance(conf, "class-course-average");
        job.setJarByClass(ClassCourseAvg.class);
